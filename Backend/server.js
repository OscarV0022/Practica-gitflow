...
app.get('/api/resumen-ventas', async (req, res) => {
  let conn;
  try {
    conn = await pool.getConnection();
...
app.put('/api/encargos/:id/pagar', async (req, res) => {
  let conn;
  try {
    conn = await pool.getConnection();
...
app.get('/', (req, res) => {
  res.send('<h1>Sistema de Restaurante: ONLINE ✅</h1><p>Si ves esto, el backend nuevo funciona.</p>');
});
